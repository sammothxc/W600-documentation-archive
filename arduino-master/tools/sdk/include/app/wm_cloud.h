...
typedef struct _CloudData
{
    int32_t totalcap;    /**< 全局缓存区的大小 4k+128byte */
    int32_t remcap;      /**< 数据包头的冗余长度 128byte */
    uint8_t *allbuf;     /**< 数据起始地址 */
    int32_t bufcap;      /**< 数据区域的大小 4K */

    uint32_t type;       /**< 消息体类型 */
    uint8_t *phead;      /**< 数据包头起始地址 */
    uint8_t *ppayload;   /**< 业务逻辑头起始地址 */
    uint8_t *pend;       /**< 数据结束地址 */
    uint8_t initiative_report;    /**<是否是主动上报 1-主动上报 0-由APP控制引发的上报 */
} CloudData,*PCloudData, CloudCallbackData, *PCloudCallbackData;
